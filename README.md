# JIGG
 ...
 ### Demos
 The current circuit in `base.js` is a 64-bit Equal-to-Zero test (`circuits/zero_equal.txt`) from [here](https://homes.esat.kuleuven.be/~nsmart/MPC/) and several other circuits from the same page.  Circuitd larger than ~5000 gates seem to hang the JS engine.  SHA256 should work in theory, but it has +100,000 gates, and JIGG will need some optimizations/throttling before it can reasonably compute a circuit that large in the browser.
