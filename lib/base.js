 ...
 *  2PC code here
 */
function garble_gate(i, j, gatetype) {
  let k = circuit[[i, j]].wireout;
  if (gatetype === 'and') {
    var t = [0,0,0,1];
  ...
  console.log([
    keyderive(Wire[i][0], Wire[j][0]), Wire[k][t[0]],
    keyderive(Wire[i][0], Wire[j][1]), Wire[k][t[1]],
    keyderive(Wire[i][1], Wire[j][0]), Wire[k][t[2]],
    keyderive(Wire[i][1], Wire[j][1]), Wire[k][t[3]]
  ]);
