...
function garble_gate(type, wirein, wireout) {
  ...
  return [
    [encrypt(Wire[i][0], Wire[j][0], k, Wire[k][t[0]]), (2*Wire[i][0].pointer()) + Wire[j][0].pointer()],
    [encrypt(Wire[i][0], Wire[j][1], k, Wire[k][t[1]]), (2*Wire[i][0].pointer()) + Wire[j][1].pointer()],
    [encrypt(Wire[i][1], Wire[j][0], k, Wire[k][t[2]]), (2*Wire[i][1].pointer()) + Wire[j][0].pointer()],
    [encrypt(Wire[i][1], Wire[j][1], k, Wire[k][t[3]]), (2*Wire[i][1].pointer()) + Wire[j][1].pointer()]
...
function evaluate_gate(gate, type, wirein, wireout) {
  ...
  if (type === 'xor') {
    ...
    Wire[k] = decrypt(Wire[i], Wire[j], k, gate[l]);
  }
  ...
  if (role === 'garbler') {
    ...
    for (var i = 0; i < circuit.gates; i++) {
      ...
      if (gate.type === 'xor') {
        ...
        Wire[k][0] = a.xor_key(b);
        Wire[k][1] = pair(
          a.xor_key(b).xor_key(R),
        ...
        let true_label = key;
        true_label.pointer(point);

        let false_label = key.xor_key(R);
        false_label.pointer(point ^ 1);

        Wire[k][0] = false_label;
        Wire[k][1] = true_label;
