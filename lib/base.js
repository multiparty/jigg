...
function evaluate_gate(gate, wirein, wireout) {
  ...
  return new Promise(function (resolve) {
    ...
    resolve(decrypt(keyderive(Wire[i], Wire[j]), gate[l]));
  });
}
...
function main() {
  ...
  if (role === 'evaluator') {
    ...
    Promise.all(messages).then(function (msg) {
      ...
      const throttle = 1;  // milliseconds
      (function evaluate_at(iii) {
        console.log('iii start', iii);
        const gatedef = circuit.gate[iii];
        evaluate_gate(gates[iii], gatedef.wirein, gatedef.wireout).then(function (labelout) {
          Wire[gatedef.wireout] = labelout;
          console.log('iii evald', iii);

          iii++; if (iii < circuit.gates) {
            console.log('iii continue', iii);
            setTimeout(() => evaluate_at(iii), throttle);
          } else {
            console.log('iii end', iii);
