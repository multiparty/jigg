...
function garble_gate(type, wirein, wireout) {
  ...
  return [
    [encrypt(Wire[i][0], Wire[j][0], k, Wire[k][t[0]]).stringify(), (2*Wire[i][0].pointer()) + Wire[j][0].pointer()],
    [encrypt(Wire[i][0], Wire[j][1], k, Wire[k][t[1]]).stringify(), (2*Wire[i][0].pointer()) + Wire[j][1].pointer()],
    [encrypt(Wire[i][1], Wire[j][0], k, Wire[k][t[2]]).stringify(), (2*Wire[i][1].pointer()) + Wire[j][0].pointer()],
    [encrypt(Wire[i][1], Wire[j][1], k, Wire[k][t[3]]).stringify(), (2*Wire[i][1].pointer()) + Wire[j][1].pointer()]
...
function evaluate_gate(gate, type, wirein, wireout) {
  ...
  if (type === 'xor') {
    ...
    Wire[k] = decrypt(Wire[i], Wire[j], k, Label(gate[l]));
  }
...
function main() {
  ...
  if (role === 'garbler') {
    // User input
    const party_input = hex2bin($('#input').val()).split('').map(JSON.parse);
    ...
    for (var i = 0; i < circuit.gates; i++) {
      let gate = circuit.gate[i];
      let label_gate = i;

      if (gate.type === 'xor') {
        let a = Wire[gate.wirein[0]][0];
        let b = Wire[gate.wirein[1]][0];
        let k = gate.wireout;

        Wire[k][0] = a.xor_key(b).point(a.pointer() ^ b.pointer());
        Wire[k][1] = a.xor_key(b).xor_key(R).point(a.pointer() ^ b.pointer() ^ 1);
        ...
        Wire[k][1] = key.xor_key(R).point(point ^ 1);
    ...
    get('evaluation').then(function (evaluation) {
      ...
      for (var i = 0; i < circuit.output.length; i++) {
        let label = evaluation[circuit.output[i]];  // wire output label
        let states = Wire[circuit.output[i]].map(Label.prototype.stringify);  // true and false labels
        let value = states.map((e) => e.substring(0, e.length-3)).indexOf(label.substring(0, label.length-3));  // find which state the label represents
        results.push(value);
      }
      give('results', results); console.log('results', results);
      $('#results').text('Results: ' + bin2hex(results.join('')));
    });
  }
      ...
      (function evaluate_at(start) {
        (new Promise(function (resolve) {
          ...
          start += parallel; if (start < circuit.gates) {
            ...
            get('results')
            .then((results) => {$('#results').text('Results: ' + bin2hex(results.join('')));return results;})
