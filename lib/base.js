...
var Wire;
function init() {
  circuit_from_path('circuits/zero_equal.txt', 'text').then((bristol) => circuit = bristol).then(main);
}
...
function main() {
  ...
  if (role === 'garbler') {
    // User input
    const inputs = (new Array(1)).concat($('#input').val().split('').map(JSON.parse)).concat(new Array(32));
  ...
  if (role === 'evaluator') {
    const inputs = (new Array(32+1)).concat($('#input').val().split('').map(JSON.parse));
    console.log('input states', inputs);
    ...
    Promise.all(messages).then(function (msg) {
      ...
      get('results').then((results) => {$('#results').text('Results: ' + results.join('')); return results;}).then(console.log.bind(null, 'results'));
    });
  }
}
