 ...
 *  Label object
 */
var label = function(){return Reflect.construct(Uint8Array, arguments, label)};// function (init = 17) {
//   console.log(init);
//   // return (function(x){return new Uint8Array.prototype.constructor(x);}).call(this, init);
//   Uint8Array.call(this);
// };
// label.superclass = Uint8Array.prototype;
// label.prototype = Object.create(label.superclass);
// label.prototype.constructor = label;

Reflect.setPrototypeOf(label.prototype, Uint8Array.prototype);
Reflect.setPrototypeOf(label, Uint8Array);
//
// // label.prototype = Object.create(Uint8Array.prototype);
// label.prototype.constructor = label;
...
label.prototype.pointer = () => this[16];
label.prototype.xor_key = function (b) {
  if (self.length !== b.length) {
    throw new error('array length mismatch');
  }
  var c = new label(a.length-1);
...
const encrypt = decrypt = function(a, b, t, m) {
  ...
  const k = 4*a + 2*b + t;
  return aes_fixed(k) ^ k ^ m;
};

function aes_fixed(m) {
  const c = m;
  for (var i = 0; i < 12; i++) {
    m = (m^(m>>Math.floor(Math.log2(m)/2)+m)) % c;
    m *= (m%3 + 1) % c;
    m = (m+(m>>Math.floor(Math.log2(m)/2)^m)) % c;
  }
