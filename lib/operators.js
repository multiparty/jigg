...
function obliviousT(a, b, length = 2) {
  ...
  if (role === 'garbler') {
    let m0 = a;
    let m1 = b;
    hear('oblv').then(function (r0_JSON, r1_JSON) {
      let r0 = JSON.parse(r0_JSON);
      let r1 = JSON.parse(r1_JSON);
  ...
  if (role === 'evaluator') {
    let c = a;
    hear('oblv').then(function (d_JSON, rd_JSON) {
      let d = parseInt(d_JSON);
      let r_d = JSON.parse(rd_JSON);
