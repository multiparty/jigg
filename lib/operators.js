...
Label.prototype.xor = function (b) {  // xor_array.bind(this, this, arguments[0], this.length-1);
  return xor_array(this, b, this.length);
};
 ...
 */
var catalog = [];
function random_no_repeat(bitLength = bytes*8) {
  var r = random_bit();
 ...
 *  Pick a random number within bitLength bits without replacement
 */
function random_with_replacement(bitLength = bytes*8) {
  var r = random_bit();
  for (var i = 1; i < bitLength; i++) {
    r *= 2;
    r += random_bit();
...
/*
 *  Fixed-key 1-block cipher as the Random Oracale
 */
...
/*
 *  Oblivious Transfer primative
 *  Sender calls obliviousT(a, b)
 *  Receiver calls obliviousT(c) and gets c?a:b
 */
function obliviousT(a, b) {
  let msg_id = nextid();
  call('oblv', {msg_id: msg_id, length: bytes+1});
