...
describe('Exhaustive regular server: logic-or-4-bit.txt', function () {
  let serverProcess;
  before(function () {
  ...
  for (let i = 0; i < inputs.length; i++) {
    for (let j = 0; j < inputs.length; j++) {
      const expectedOutput = (i === 0 && j === 0) ? '[ 0 ]' : '[ 1 ]';
      const garblerInput = inputs[i];
      const evaluatorInput = inputs[j];

      it('Inputs ' + garblerInput + ' ' + evaluatorInput, async function () {
        const garbler = new Process('node', ['demo/party.js', '3001', 'Garbler', garblerInput, 'bits', 'logic-or-4-bit.txt', 'false']);
        const evaluator = new Process('node', ['demo/party.js', '3001', 'Evaluator', evaluatorInput, 'bits', 'logic-or-4-bit.txt', 'false']);
