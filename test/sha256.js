...
describe('SHA256 Garbling Server', function () {
  ...
  for (let i = 0; i < inputs.length; i++) {
    ...
    it('Input ' + i, async function () {
      // run garbling server
      const garbler = new Process('node', ['demo/sha256/server.js', 'Garbler', garblerInput, 'false']);
      // run evaluator party
      const evaluator = new Process('node', ['demo/sha256/party.js', 'Evaluator', evaluatorInput, 'false']);
